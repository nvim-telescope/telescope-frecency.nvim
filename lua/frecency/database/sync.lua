...
---@class FrecencySyncDatabase: FrecencyDatabase
local Sync = {}

---@param fs FrecencyFS
---@param config FrecencyDatabaseConfig
---@return FrecencyDatabase
Sync.new = function(fs, config)
  local version = "v1"
...
---@return boolean
function Sync:has_entry()
  return not vim.tbl_isempty(self.table.records)
end

---@param paths string[]
---@return nil
function Sync:insert_files(paths)
  if #paths == 0 then
    return
  end
...
---@return string[]
function Sync:unlinked_entries()
  local paths = {}
...
---@param paths string[]
function Sync:remove_files(paths)
...
function Sync:get_entries(workspace, datetime)
  local now = self:now(datetime)
  local items = {}
...
---@param path string
---@return boolean
function Sync:remove_entry(path)
